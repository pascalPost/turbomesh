    ...
    fn fillAndSolve(self: *RowCompressedMatrixSystem2d) !void {
        ...
        for (self.mesh.blocks.items) |b| {
            ...
            // corner A[0, 0] = (1, 1)
            {
                ...
                lhs_values[matrix_idx + 0] = stencil.get(.i_j); // A[i, j]
                lhs_values[matrix_idx + 1] = stencil.get(.i_jp1); // A[i, j+1]
                lhs_values[matrix_idx + 2] = stencil.get(.ip1_j); // A[i+1, j]
                lhs_values[matrix_idx + 3] = stencil.get(.ip1_jp1); // A[i+1, j+1]

                matrix_idx += 4;

                // RHS: A[i-1, j-1], A[i-1, j], A[i-1, j+1], A[i, j-1] A[i+1, j-1]

                rhs_x[rhs_idx + 0] = -(stencil.get(.im1_jm1) * im1_jm1.data[0] +
                    stencil.get(.im1_j) * im1_j.data[0] +
                    stencil.get(.im1_jp1) * im1_jp1.data[0] +
                    stencil.get(.i_jm1) * i_jm1.data[0] +
                    stencil.get(.ip1_jm1) * ip1_jm1.data[0]);

                rhs_y[rhs_idx + 0] = -(stencil.get(.im1_jm1) * im1_jm1.data[1] +
                    stencil.get(.im1_j) * im1_j.data[1] +
                    stencil.get(.im1_jp1) * im1_jp1.data[1] +
                    stencil.get(.i_jm1) * i_jm1.data[1] +
                    stencil.get(.ip1_jm1) * ip1_jm1.data[1]);
            ...
            for (2..points.size[1] - 2) |_| {
                ...
                lhs_values[matrix_idx + 0] = stencil.get(.i_jm1); // A[i, j-1]
                lhs_values[matrix_idx + 1] = stencil.get(.i_j); // A[i, j]
                lhs_values[matrix_idx + 2] = stencil.get(.i_jp1); // A[i, j+1]
                lhs_values[matrix_idx + 3] = stencil.get(.ip1_jm1); // A[i+1, j-1]
                lhs_values[matrix_idx + 4] = stencil.get(.ip1_j); // A[i+1, j]
                lhs_values[matrix_idx + 5] = stencil.get(.ip1_jp1); // A[i+1, j+1]

                matrix_idx += 6;

                // RHS: A[i-1, j-1], A[i-1, j], A[i-1, j+1]

                rhs_x[rhs_idx] = -(stencil.get(.im1_jm1) * im1_jm1.data[0] +
                    stencil.get(.im1_j) * im1_j.data[0] +
                    stencil.get(.im1_jp1) * im1_jp1.data[0]);

                rhs_y[rhs_idx] = -(stencil.get(.im1_jm1) * im1_jm1.data[1] +
                    stencil.get(.im1_j) * im1_j.data[1] +
                    stencil.get(.im1_jp1) * im1_jp1.data[1]);
            ...
            // corner A[0, m] = (1, J-2)
            {
                ...
                lhs_values[matrix_idx + 0] = stencil.get(.i_jm1); // A[i, j-1]
                lhs_values[matrix_idx + 1] = stencil.get(.i_j); // A[i, j]
                lhs_values[matrix_idx + 2] = stencil.get(.ip1_jm1); // A[i+1, j-1]
                lhs_values[matrix_idx + 3] = stencil.get(.ip1_j); // A[i+1, j]
                ...
                rhs_x[rhs_idx] = -(stencil.get(.im1_jm1) * im1_jm1.data[0] +
                    stencil.get(.im1_j) * im1_j.data[0] +
                    stencil.get(.im1_jp1) * im1_jp1.data[0] +
                    stencil.get(.i_jp1) * i_jp1.data[0] +
                    stencil.get(.ip1_jp1) * ip1_jp1.data[0]);

                rhs_y[rhs_idx] = -(stencil.get(.im1_jm1) * im1_jm1.data[1] +
                    stencil.get(.im1_j) * im1_j.data[1] +
                    stencil.get(.im1_jp1) * im1_jp1.data[1] +
                    stencil.get(.i_jp1) * i_jp1.data[1] +
                    stencil.get(.ip1_jp1) * ip1_jp1.data[1]);
            ...
            for (2..points.size[0] - 2) |_| {
                ...
                // edge A[i, 0] = (i, 0)
                {
                    ...
                    lhs_values[matrix_idx + 0] = stencil.get(.im1_j); // A[i-1, j]
                    lhs_values[matrix_idx + 1] = stencil.get(.im1_jp1); // A[i-1, j+1]
                    lhs_values[matrix_idx + 2] = stencil.get(.i_j); // A[i, j]
                    lhs_values[matrix_idx + 3] = stencil.get(.i_jp1); // A[i, j+1]
                    lhs_values[matrix_idx + 4] = stencil.get(.ip1_j); // A[i+1, j]
                    lhs_values[matrix_idx + 5] = stencil.get(.ip1_jp1); // A[i+1, j+1]
                ...
                for (2..points.size[1] - 2) |_| {
                    ...
                    lhs_values[matrix_idx + 0] = stencil.get(.im1_jm1); // A[i-1, j-1]
                    lhs_values[matrix_idx + 1] = stencil.get(.im1_j); // A[i-1, j]
                    lhs_values[matrix_idx + 2] = stencil.get(.im1_jp1); // A[i-1, j+1]
                    lhs_values[matrix_idx + 3] = stencil.get(.i_jm1); // A[i, j-1]
                    lhs_values[matrix_idx + 4] = stencil.get(.i_j); // A[i, j]
                    lhs_values[matrix_idx + 5] = stencil.get(.i_jp1); // A[i, j+1]
                    lhs_values[matrix_idx + 6] = stencil.get(.ip1_jm1); // A[i+1, j-1]
                    lhs_values[matrix_idx + 7] = stencil.get(.ip1_j); // A[i+1, j]
                    lhs_values[matrix_idx + 8] = stencil.get(.ip1_jp1); // A[i+1, j+1]
                ...
                // edge A[i, m] = (i, J-2)
                {
                    ...
                    lhs_values[matrix_idx + 0] = stencil.get(.im1_jm1); // A[i-1, j-1]
                    lhs_values[matrix_idx + 1] = stencil.get(.im1_j); // A[i-1, j]
                    lhs_values[matrix_idx + 2] = stencil.get(.i_jm1); // A[i, j-1]
                    lhs_values[matrix_idx + 3] = stencil.get(.i_j); // A[i, j]
                    lhs_values[matrix_idx + 4] = stencil.get(.ip1_jm1); // A[i+1, j-1]
                    lhs_values[matrix_idx + 5] = stencil.get(.ip1_j); // A[i+1, j]

                    matrix_idx += 6;

                    // RHS: A[i-1, j+1], A[i, j+1], A[i+1, j+1]

                    rhs_x[rhs_idx] = -(stencil.get(.im1_jp1) * im1_jp1.data[0] +
                        stencil.get(.i_jp1) * i_jp1.data[0] +
                        stencil.get(.ip1_jp1) * ip1_jp1.data[0]);

                    rhs_y[rhs_idx] = -(stencil.get(.im1_jp1) * im1_jp1.data[1] +
                        stencil.get(.i_jp1) * i_jp1.data[1] +
                        stencil.get(.ip1_jp1) * ip1_jp1.data[1]);
            ...
            // corner A[n, 0] = (I-2, 1)
            {
                ...
                rhs_x[rhs_idx] = -(stencil.get(.ip1_jm1) * ip1_jm1.data[0] +
                    stencil.get(.ip1_j) * ip1_j.data[0] +
                    stencil.get(.ip1_jp1) * ip1_jp1.data[0] +
                    stencil.get(.i_jm1) * i_jm1.data[0] +
                    stencil.get(.im1_jm1) * im1_jm1.data[0]);

                rhs_y[rhs_idx] = -(stencil.get(.ip1_jm1) * ip1_jm1.data[1] +
                    stencil.get(.ip1_j) * ip1_j.data[1] +
                    stencil.get(.ip1_jp1) * ip1_jp1.data[1] +
                    stencil.get(.i_jm1) * i_jm1.data[1] +
                    stencil.get(.im1_jm1) * im1_jm1.data[1]);
        ...
        for (self.mesh.connections.items) |connection| {
            ...
            const matrix_idx_inc_extern_0: [2]usize = if (it.in_connection_direction_shift[0] > 0) .{ 0, 1 } else .{ 1, 0 };
            const matrix_idx_inc_extern_1: [2]usize = if (it.in_connection_direction_shift[1] > 0) .{ 2, 3 } else .{ 3, 2 };
            ...
            // first internal point has first point fixed.
            {
                ...
                lhs_values[matrix_idx + matrix_idx_inc_extern_0[0]] = stencil.get(.i_jm1); // A[matrix_idx, matrix_idx_0]
                lhs_values[matrix_idx + matrix_idx_inc_extern_0[1]] = stencil.get(.ip1_jm1); // A[matrix_idx, matrix_idx_0 + matrix_increment_0]
                lhs_values[matrix_idx + matrix_idx_inc_extern_1[0]] = stencil.get(.i_jp1); // A[matrix_idx, matrix_idx_1]
                lhs_values[matrix_idx + matrix_idx_inc_extern_1[1]] = stencil.get(.ip1_jp1); // A[matrix_idx, matrix_idx_1 + matrix_increment_1]
                lhs_values[matrix_idx + 4] = stencil.get(.i_j); // A[matrix_idx, matrix_idx]
                lhs_values[matrix_idx + 5] = stencil.get(.ip1_j); // A[matrix_idx, matrix_idx + 1]

                matrix_idx += 6;

                // RHS: fixed first point of the connection including its neighbors (i-1,j-1) (i-1,j) (i-1,j+1)

                rhs_x[rhs_idx + 0] = -(stencil.get(.im1_jm1) * im1_jm1.data[0] + stencil.get(.im1_j) * im1_j.data[0] + stencil.get(.im1_jp1) * im1_jp1.data[0]);
                rhs_y[rhs_idx + 0] = -(stencil.get(.im1_jm1) * im1_jm1.data[1] + stencil.get(.im1_j) * im1_j.data[1] + stencil.get(.im1_jp1) * im1_jp1.data[1]);
            ...
            for (0..it.count - 1) |_| {
                ...
                lhs_values[matrix_idx + matrix_idx_inc_intern_0[0]] = stencil.get(.im1_jm1); // A[matrix_idx, matrix_idx_0 - matrix_increment_0]
                lhs_values[matrix_idx + matrix_idx_inc_intern_0[1]] = stencil.get(.i_jm1); // A[matrix_idx, matrix_idx_0]
                lhs_values[matrix_idx + matrix_idx_inc_intern_0[2]] = stencil.get(.ip1_jm1); // A[matrix_idx, matrix_idx_0 + matrix_increment_0]

                lhs_values[matrix_idx + matrix_idx_inc_intern_1[0]] = stencil.get(.im1_jp1); // A[matrix_idx, matrix_idx_1 - matrix_increment_1]
                lhs_values[matrix_idx + matrix_idx_inc_intern_1[1]] = stencil.get(.i_jp1); // A[matrix_idx, matrix_idx_1]
                lhs_values[matrix_idx + matrix_idx_inc_intern_1[2]] = stencil.get(.ip1_jp1); // A[matrix_idx, matrix_idx_1 + matrix_increment_1]

                lhs_values[matrix_idx + 6] = stencil.get(.im1_j); // A[matrix_idx, matrix_idx - 1]
                lhs_values[matrix_idx + 7] = stencil.get(.i_j); // A[matrix_idx, matrix_idx]
                lhs_values[matrix_idx + 8] = stencil.get(.ip1_j); // A[matrix_idx, matrix_idx + 1]
            ...
            // second to last point has last point fixed.
            {
                ...
                lhs_values[matrix_idx + matrix_idx_inc_extern_0[0]] = stencil.get(.im1_jm1); // A[matrix_idx, matrix_idx_0 - matrix_increment_0]
                lhs_values[matrix_idx + matrix_idx_inc_extern_0[1]] = stencil.get(.i_jm1); // A[matrix_idx, matrix_idx_0]
                lhs_values[matrix_idx + matrix_idx_inc_extern_1[0]] = stencil.get(.im1_jp1); // A[matrix_idx, matrix_idx_1 - matrix_increment_1]
                lhs_values[matrix_idx + matrix_idx_inc_extern_1[1]] = stencil.get(.i_jp1); // A[matrix_idx, matrix_idx_1]
                lhs_values[matrix_idx + 4] = stencil.get(.im1_j); // A[matrix_idx, matrix_idx - 1]
                lhs_values[matrix_idx + 5] = stencil.get(.i_j); // A[matrix_idx, matrix_idx]

                matrix_idx += 6;

                // RHS: fixed first point of the connection including its neighbors (i-1,j-1) (i-1,j) (i-1,j+1)

                rhs_x[rhs_idx + 0] = -(stencil.get(.ip1_jm1) * ip1_jm1.data[0] + stencil.get(.ip1_j) * ip1_j.data[0] + stencil.get(.ip1_jp1) * ip1_jp1.data[0]);
                rhs_y[rhs_idx + 0] = -(stencil.get(.ip1_jm1) * ip1_jm1.data[1] + stencil.get(.ip1_j) * ip1_j.data[1] + stencil.get(.ip1_jp1) * ip1_jp1.data[1]);
...
pub fn mesh(allocator: std.mem.Allocator, mesh_data: *discrete.Mesh, iterations: usize) !void {
    ...
    for (0..iterations) |n| {
        ...
        // TODO put this also in the system matrix
        // Laplacian smoothing of the junction points
        {
            // TODO remove this hard-coding

            const idx_i_j = [_]struct { block: usize, point_idx: types.Index2d }{
                .{ .block = 2, .point_idx = .{ 53, 115 } },
                .{ .block = 4, .point_idx = .{ 0, 0 } },
                .{ .block = 6, .point_idx = .{ 20, 20 } },
            };

            std.debug.assert(types.eqlApprox(mesh_data.blocks.items[idx_i_j[0].block].points.getIndex(idx_i_j[0].point_idx), mesh_data.blocks.items[idx_i_j[1].block].points.getIndex(idx_i_j[1].point_idx), 1e-10));
            std.debug.assert(types.eqlApprox(mesh_data.blocks.items[idx_i_j[0].block].points.getIndex(idx_i_j[0].point_idx), mesh_data.blocks.items[idx_i_j[2].block].points.getIndex(idx_i_j[2].point_idx), 1e-10));

            const x_im1_j = mesh_data.blocks.items[6].points.getIndex(.{ 20, 19 });
            const x_ip1_j = mesh_data.blocks.items[6].points.getIndex(.{ 20, 21 });
            const x_i_jp1 = mesh_data.blocks.items[4].points.getIndex(.{ 1, 0 });
            const x_im1_jp1 = mesh_data.blocks.items[4].points.getIndex(.{ 1, 1 });
            const x_ip1_jp1 = mesh_data.blocks.items[2].points.getIndex(.{ 52, 114 });
            const x_i_jm1 = mesh_data.blocks.items[6].points.getIndex(.{ 19, 20 });
            const x_im1_jm1 = mesh_data.blocks.items[6].points.getIndex(.{ 19, 19 });
            const x_ip1_jm1 = mesh_data.blocks.items[6].points.getIndex(.{ 19, 21 });

            const x_i_j_new = types.scale(1.0 / 8.0, types.addAll([_]types.Vec2d{ x_im1_j, x_ip1_j, x_i_jp1, x_im1_jp1, x_ip1_jp1, x_i_jm1, x_im1_jm1, x_ip1_jm1 }));

            // mesh_data.blocks
            for (idx_i_j) |item| {
                const points = mesh_data.blocks.items[item.block].points;
                points.data[points.index(item.point_idx)] = x_i_j_new;
            }
        }
    }
}
