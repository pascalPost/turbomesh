...
// TODO: create CGNS struct with functions to add grid, solutions etc.

pub fn write(filename: [:0]const u8, block_names: []const [:0]const u8, block_points: []const Mat2d, buffer: []Float, control_function: ?[]types.Vec2d) !void {
    ...
    for (block_points, 0..) |block, i_block| {
        ...
        if (control_function) |cf| {
            var solution_handle: c_int = undefined;
            ierr = cgns.cg_sol_write(file_handle, base_handle, zone_handle, "Smoothing", cgns.Vertex, &solution_handle);

            if (ierr != 0) {
                cgns_log.err("error writing solution (file: {}, base: {}, zone: {}, sol: {}): {s}", .{ file_handle, base_handle, zone_handle, solution_handle, getErrorMessage() });
                return error.cgnsCoordWrite;
            }

            var field_handle: c_int = undefined;

            var block_range_start: usize = 0;
            for (block_points[0..i_block]) |b| block_range_start += b.size[0] * b.size[1];

            {
                ...
                while (j < block.size[1]) : (j += 1) {
                    var i: Index = 0;
                    while (i < block.size[0]) : (i += 1) {
                        buffer[idx] = cf[block_range_start + block.index(.{ i, j })].data[0];
                        idx += 1;
                    }
                }
            }

            ierr = cgns.cg_field_write(file_handle, base_handle, zone_handle, solution_handle, cgns.RealDouble, "P", buffer.ptr, &field_handle);

            if (ierr != 0) {
                cgns_log.err("error writing field (file: {}, base: {}, zone: {}, sol: {}, field: {}): {s}", .{ file_handle, base_handle, zone_handle, solution_handle, field_handle, getErrorMessage() });
                return error.cgnsCoordWrite;
            }

            {
                ...
                while (j < block.size[1]) : (j += 1) {
                    var i: Index = 0;
                    while (i < block.size[0]) : (i += 1) {
                        buffer[idx] = cf[block_range_start + block.index(.{ i, j })].data[1];
                        idx += 1;
                    }
                }
            }

            ierr = cgns.cg_field_write(file_handle, base_handle, zone_handle, solution_handle, cgns.RealDouble, "Q", buffer.ptr, &field_handle);
